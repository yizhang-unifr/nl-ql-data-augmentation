...
def _inference_semql(data_row, schemas, model, beam_size):
    ...
    # in case someone is requesting more beams, we assemble them here as well.
        for beam in results:
            ...
            all_beams.append(
                (" ".join([str(x) for x in beam.actions]), beam.score.item() / beam.t))
...
def _pre_processing(example, db_value_finder, ner_api_secret):
    ner_information = remote_named_entity_recognition(
        example['question'], ner_api_secret)

    token_grouped, token_types, column_matches, value_candidates, _ = pre_process(
        0, example, ner_information, db_value_finder, is_training=False)
...
def _execute_query_postgresql(sql, database, connection_config, limit_rows=100):
    ...
    cursor = conn.cursor()
    # optimize performance by limiting the size of rows returned.
    sql = ' '.join([sql.strip(), f'LIMIT {limit_rows}'])
    try:
        cursor.execute(sql)
    # result = cursor.fetchall()
    # only retruen 10 results for better performance
        ...
        print("********************************")
        print("Exception")
        print("Errors occured when fetch results of queries")
...
def get_schema_sdss():
    base_path = Path(Config.DATA_PREFIX) / \
        'skyserver_dr16_2020_11_30' / 'original'
    schema_path = str(base_path / 'tables.json')

    schemas_raw, schemas_dict = spider_utils.load_schema(schema_path)
    return schemas_raw, schemas_dict, schema_path

def get_schemas_worldcup():
...
def get_data_folder_by_database(database_name):
    ...
    if database_name == 'oncomx_v1_0_25_small':
        return 'oncomx'
    if database_name == 'skyserver_dr16_2020_11_30':
        return 'skyserver_dr16_2020_11_30'
    else:
        return 'spider'
