    ...
    def __init__(self, query):
        if isinstance(query, dict):
            assert sorted(query.keys()) == [
                "params",
                "pattern",
            ], "dict must have and only have keys 'params' and 'pattern'"
...
class DatabaseValueSamplerPostgresSQL(DatabaseValueSampler):
    def __init__(
        self, database_name, db_schema_information, connection_config, max_results=100
    ):
        ...
        self.db_options = (
            f"-c search_path={connection_config['database_schema']},public"
        )
