...
from intermediate_representation.semQL import C, T, A, V, Op
    ...
    def __init__(self, question_tokens, semql_actions=None, column_tokens=None, n_columns=None, sql=None, column_matches=None,
        ...
        if self.semql_actions:
            for action in self.semql_actions:
                if isinstance(action, C) or isinstance(action, T) or isinstance(action, A) or isinstance(action, V) or isinstance(action, Op):
                    continue
...
class Batch(object):
    def __init__(self, examples, grammar, cuda=False):
        ...
        if examples[0].semql_actions:
            self.max_action_num = max(len(e.semql_actions)
        ...
        self.all_question_tokens_len = [
            len(e.question_tokens) + 1 for e in self.examples]
