...
def build_example(sql, all_schemas):
    ...
    column_names, column_set, column_table_dict, columns_per_table, table_names = build_table_column_mappings(
        sql, schema)
...
def build_table_column_mappings(sql, table):
    ...
    column_table_dict = _get_column_table_dict(
        column_names, column_table_indices, sql)

    # a simple list with sublists for each table, containing all the columns in that table.
    columns_per_table = _get_column_sublists_per_table(
        column_table_indices, columns)
...
def _instantiate_actions(column_table_dict, sql):
    ...
    if 'rule_label' in sql:
        ...
        if is_valid(rule_label, col_table_dict=column_table_dict, sql=sql) is False:
            raise RuntimeError(
                "Invalid rule_label: {}. We don't use this sample".format(sql['rule_label']))
    return rule_label
...
def _get_column_table_dict(tab_cols, tab_ids, sql):
    ...
    for c_id, c_v in enumerate(sql['col_set']):
        for cor_id, cor_val in enumerate(tab_cols):
            if c_v == cor_val:
                table_dict[tab_ids[cor_id]] = table_dict.get(
                    tab_ids[cor_id], []) + [c_id]
...
def is_valid(rule_label, col_table_dict, sql):
    try:
        # here we rebuild a tree from the ist with rules
        lf.build_tree(copy.copy(rule_label))
    except:
        print("NOT VALID RULE LABEL", rule_label)

    flag = False
    for r_id, rule in enumerate(rule_label):
        if type(rule) == C:
            try:
                assert rule_label[r_id +
                                  1].id_c in col_table_dict[rule.id_c], print(sql['question'])
            except:
                flag = True
                print(sql['question'])
