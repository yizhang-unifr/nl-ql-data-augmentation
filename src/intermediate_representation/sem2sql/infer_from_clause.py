def infer_from_clause(table_names, graph, columns, readable_alias):
    tables = list(table_names.keys())

    if len(tables) == 1:  # no JOINS needed - just return the simple "FROM" clause.
        if readable_alias:
            return f"FROM {tables[0]} AS {table_names[tables[0]]}"
        return f"FROM {tables[0]} AS {table_names[tables[0]]}"
    else:  # we have to deal with multiple tables - and find the shortest path between them
        join_clauses, cross_join_clauses = generate_path_by_graph(graph, table_names, tables, readable_alias=readable_alias)
        ...
        if idx == 0:
            if readable_alias:
                stringified_join_clauses.append(
                    f"{start} AS {start_alias} JOIN {end} AS {end_alias} ON {start_alias}.{entry_column} = {end_alias}.{exit_column}")
            else:
                stringified_join_clauses.append(f"{start} AS {start_alias} JOIN {end} AS {end_alias} ON {start_alias}.{entry_column} = {end_alias}.{exit_column}")
        else:
            if readable_alias:
                stringified_join_clauses.append(f"JOIN {end} AS {end_alias} ON {start_alias}.{entry_column} = {end_alias}.{exit_column}")
            else:
                stringified_join_clauses.append(f"JOIN {end} AS {end_alias} ON {start_alias}.{entry_column} = {end_alias}.{exit_column}")
    ...
    for table, table_alias in cross_join_clauses:
        if len(stringified_join_clauses) == 0:
            if readable_alias:
                ...
                stringified_join_clauses.append(f"{table} AS {table_alias}")
        else:
            if readable_alias:
                stringified_join_clauses.append(f"JOIN {table}")
            else:
                stringified_join_clauses.append(f"JOIN {table} AS {table_alias}")

    return f'FROM {" ".join(stringified_join_clauses)}'


def generate_path_by_graph(graph, table_names, tables, readable_alias):
    join_clause = list()
...
def _get_max_alias(table_names):
    ...
    for t, k in table_names.items():
        ...
        if _k > max_key:
            max_key = _k
    # return max_key + 10    # ?from Yi: Why add 10?
    return max_key
