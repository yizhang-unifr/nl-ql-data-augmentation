...
from intermediate_representation.semQL import Sup, Sel, Order, Root, Filter, A, N, C, T, V, Root1, Op
...
def _build_single_filter(lf, f):
    ...
    if len(lf) == 0:
        ...
        table_1 = lf.pop(0)
        if not isinstance(table_1, T):
            lf.insert(0, table_1)
            table_1 = None
    assert isinstance(agg, A) and isinstance(op, Op)
    ...
    if 2 <= f.id_c <= 10:
        ...
        agg.add_children(op)
        op.set_parent(agg)
        op.add_children(column_1)
        column_1.set_parent(op)
        if table_1 is not None:
            column_1.add_children(table_1)
            table_1.set_parent(column_1)
        if len(lf) > 0 and isinstance(lf[0], C):
            column_2 = lf.pop(0)
            if len(lf) == 0:
                lf.insert(0, column_2)
                column_2 = None
            else:
                table_2 = lf.pop(0)
                op.add_children(column_2)
                column_2.set_parent(op)
                column_2.add_children(table_2)
                table_2.set_parent(column_2)
        value1 = lf.pop(0)
        value1.set_parent(f)
    ...
    while len(root.children) != length:
        ...
        if isinstance(c_instance, Sel):
            ...
            for i in range(c_instance.id_c + 1):
                ...
                assert isinstance(agg, A) and isinstance(op, Op) and isinstance(column_1, C)
...
def build_adjacency_matrix(lf, symmetry=False):
    _lf = list()
    for rule in lf:
        if isinstance(rule, A) or isinstance(rule, Op) or isinstance(rule, C) or isinstance(rule, T):
            continue
        _lf.append(rule)
    length = len(_lf)
