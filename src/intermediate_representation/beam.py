...
from intermediate_representation.semQL import Root1, Filter, N, Root, T, C, A, Sel, Op
...
class Beams(object):
    ...
    def check_sel_valid(self, actions):
        ...
        for ac in actions:
            if type(ac) == Sel:
                find_sel = True
            elif find_sel and type(ac) in [N, T, C, A, Op]:
                if type(ac) not in [N]:
                    sel_actions.append(ac)
            elif find_sel and type(ac) not in [N, T, C, A, Op]:
                break
        ...
        sel_string = list()
        for i in range(len(sel_actions) // 3): #TODO try // 6 
